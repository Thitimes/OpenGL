...
def reshape(w, h):
    ...
    glLoadIdentity()
    promat = Perspective(60, win_w/win_h, 0.01, 10)
    glLoadMatrixf(promat.T)
...
def display():
    ...
    glLoadIdentity()
    viewmat = LookAt(*eye_pos, *centroid, 0, 1, 0)
    modelmat = Rotate(tick, 0, 1, 0)
    glLoadMatrixf((viewmat @ modelmat).T)
    glUseProgram(prog_id)
