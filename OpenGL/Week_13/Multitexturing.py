...
def load_texture(filename, texture_unit):
   ...
   return texture_unit
def draw_gui():
    global I, Ka, Kd, Kd_b, Ks, shininess, clear_color,blend
    ...
    _, blend = imgui.slider_float("blend", blend, 0, 1)     
...
def display():
    ...
    bun = glGetUniformLocation(prog_id,"bunny")
    glUniform1i(bun,1)
    bri = glGetUniformLocation(prog_id,"brick")
    glUniform1i(bri,2)
    glUniform1f(glGetUniformLocation(prog_id, "blend"),blend)
...
def initialize():
    ...
    load_texture("../texture_map/bunny_hair.jpg",1)
    load_texture("../texture_map/brick_wall_small.jpg",2)
...
void main()
{
    vec3 color = mix(texture(bunny,texCoord).rgb,texture(brick,texCoord * 5).rgb,blend);
    ...
    vec3 ambient = Ka * Il;
    vec3 diffuse;
    diffuse = color * max(dot(N, L), 0) * Il;
